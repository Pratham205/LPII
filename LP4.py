N = 8  # assuming N is set to 8 for an 8x8 chessboard
...
def isSafe(row, col, slashCode, backslashCode, rowLookup, slashCodeLookup, backslashCodeLookup):
    if (not slashCodeLookup[slashCode[row][col]] and not backslashCodeLookup[backslashCode[row][col]] and not rowLookup[row]):
        return True
    return False
